...
def test_max_main():
    ...
    dataset = FieldCutData(root_dir='openfwi', )
...
def test1():
    # 创建数据集实例
    dataset = FieldCutData(root_dir='data1_cut', use_data=('depth_vel', 'migrated_image', 'well_log', 'horizon', 'rms_vel'), use_normalize='-1_1')
    # 创建 DataLoader 实例
...
def test_max_min():
    ...
    migrated_image_list = []
    for i in range(144):
        depth_vel_list.append(np.load(f'data1_cut/depth_vel/{i}.npy').squeeze())
        migrated_image_list.append(np.load(f'data1_cut/migrated_image/{i}.npy').squeeze())
    depth_vel = np.stack(depth_vel_list)
    migrated_image = np.stack(migrated_image_list)
    print(depth_vel.max(), depth_vel.min())
    print(migrated_image.max(), migrated_image.min())
