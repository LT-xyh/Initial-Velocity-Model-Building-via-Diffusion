...
class AutoencoderLightning(BaseLightningModule):
    def __init__(self, conf):
        super().__init__(batch_size=conf.training.dataloader.batch_size, lr=conf.training.lr)
        self.conf = conf
        self.ae = MyAE(in_channels=self.conf.datasets.depth_velocity.shape[0],
                       latent_channels=self.conf.autoencoder_conf.latent_channels,
                       in_size=self.conf.datasets.depth_velocity.shape[1:],
                       base_channel=64)
