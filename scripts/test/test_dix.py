...
from lightning_modules.baselines_lightning.DixLightning import DixLightning
...
def base_test_dix(model, test_set, batch_size, log_dir, log_version, fast_run=False):
    ...
    tensorboard_logger = TensorBoardLogger(save_dir=log_dir, name='tensorboard',
                                           version=log_version, )
    ...
    trainer = lightning.Trainer(precision='bf16-mixed',  # fp16混合精度训练
                                ...
                                log_every_n_steps=512 // batch_size,
                                fast_dev_run=fast_run,
                                )
    trainer.test(model, dataloaders=test_loader)


def test_dix_OpenFWI(batch_size, log_dir, log_version):
...
def test_dix_CurveFaultA(batch_size, log_dir, log_version):
